...
{
    public partial class DoanhthuHDBV : Form
    {
        public DoanhthuHDBV()
        {
            InitializeComponent();
            // Gán sự kiện CheckedChanged cho các RadioButton
            rbtTheongay.CheckedChanged += new EventHandler(RadioButton_CheckedChanged);
            rbtTheokhoang.CheckedChanged += new EventHandler(RadioButton_CheckedChanged);
        }
        private void RadioButton_CheckedChanged(object sender, EventArgs e)
        {
            if (rbtTheongay.Checked)
            {
                mskTheongay.Enabled = true;
                mskTheokhoang.Enabled = false;
                mskDenkhoang.Enabled = false;
            }
            else if (rbtTheokhoang.Checked)
            {
                mskTheongay.Enabled = false;
                mskTheokhoang.Enabled = true;
                mskDenkhoang.Enabled = true;
            }
        }
        ...
        private void BaocaoKhanh_Load(object sender, EventArgs e)
        {
            ...
            Class.Function.FillCombo("Select MaQCao, TenQCao from tblTTQuangcao", cboQuangcao, "MaQcao", "MaQCao");
            cboNhanvien.SelectedIndex = -1;
            Class.Function.FillCombo("Select MaNV, TenNV from tblNhanvien", cboNhanvien, "MaNV", "MaNV");
            cboQuangcao.SelectedIndex = -1;
        ...
        {
            string sql;
            if (cboLoaibao.SelectedValue == null  && cboQuangcao.SelectedValue == null && cboNhanvien.SelectedValue == null && txtKhachhang.Text == "" && mskTheongay.Text == "  /  /" && txtDoanhthu.Text == "" && mskTheokhoang.Text == "  /  /" && mskDenkhoang.Text == "  /  /")
            {
            ...
            if (cboQuangcao.SelectedValue != null)
            {
                sql += " AND MaQcao = N'" + cboQuangcao.SelectedValue.ToString() + "'";
            }
            ...
            {
                sql += " AND MaNV = N'" + cboNhanvien.SelectedValue.ToString() + "'";
            }
        ...
        private void load_datagrid(string sql)
        {
            try
            {
                ...
                dgridDoanhthu.Columns[4].HeaderText = "Mã QC";
                dgridDoanhthu.Columns[5].HeaderText = "Nội dung";
                dgridDoanhthu.Columns[6].HeaderText = "Ngày BĐ";
                dgridDoanhthu.Columns[7].HeaderText = "Ngày KT";
        ...
        private void btxBoqua_Click(object sender, EventArgs e)
        {
            ResetValues();

            btxBoqua.Enabled = true;
        }
        ...
        private void btnInbaocao_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                exRange.Range["A11:N11"].Font.Bold = true;
                exRange.Range["A11:N11"].HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Range["A11:n11"].ColumnWidth = 12;
